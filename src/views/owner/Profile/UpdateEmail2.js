...
function UpdateProfile(props) {
    ...
    const handleResend = async (e, err, value) => {
        ...
        try {
            ...
            const q3 = {
                query: `mutation{
                    otp(email:"${props.data.email}", emailBaru:"${props.data.emailBaru}"){
                        code
                    }
                }`
            }
    ...
    const handleSubmit2 = async (e, err, value) => {
        ...
        try {
            ...
            const q2 = {
                query: `mutation{
                    updateEmail(email:"${props.data.email}", emailBaru:"${props.data.emailBaru}")
                }`
            }
            ...
            showNotification("Berhasil memperbaharui email", "success");
            setTimeout(() => {
                props.onSuccess();
            }, 4000);
        } catch (e) {
    ...
    return (
        <CreateModal modal={props.modal} title={'Verifikasi OTP'} onCancel={props.onCancel}
            ...
            <div className="text-center">
                <p style={{color:"red", fontSize:"10px"}}>Kode OTP dikirimkan ke email baru anda, dan valid selama 1 jam</p>
            </div>
        </CreateModal>
    )
}
