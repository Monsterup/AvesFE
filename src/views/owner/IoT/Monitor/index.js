...
function Alat() {
    ...
    //Data Table
    const objectName = "Monitor IoT";
    const headings = ['Nomor Seri', 'Tanggal', 'Tipe', 'Hasil'];
    const columns = ['device', 'date', 'sensorType', 'value'];
    ...
    const Modal = () => {
        return (
            <div>
                <CreateMonitor objectName={objectName} onSuccess={() => fetchData(keyword, limit, skip, () => {})}/>
                {updateModal && 
                <UpdateMonitor objectName={objectName} onSuccess={updateSuccess} data={object} modal={updateModal}
    ...
    const fetchData = (keyword = '', limit_ = limit, skip_ = skip, callback) => {
        const q = `query{
                        deviceRecords(keyword : "${keyword}", limit : ${limit_}, skip : ${skip_}){
                            totalCount
                            deviceRecords{
                                _id
                                value
        ...
        AsyncFetch(q, (res) => {
            console.log(res);
            setLoading(false);
            setObjects(res.data.data.deviceRecords.deviceRecords);
            const page = Math.ceil(parseInt(res.data.data.deviceRecords.totalCount) / limit_);
            callback(page);
        })
    }
    ...
    const showUpdateDialog = (_id) => {
        ...
        AsyncFetch(q, (res) => {
            console.log(res);
            result.deviceRecord = res.data.data.deviceRecord;
        });
        AsyncFetch(q2, (res) => {
            result.device = res.data.data.devices.devices;
            setObject(result);
        });
    }
    ...
    const confirmDelete = () => {
        const q = `mutation{
                    deleteDeviceRecord(_id : "${deleteObject}"){
                        deleted
                    }
                }
            `;
    ...
    const renderData = (objects) => {
        return objects.map(function (row, row_index) {
            row.index = row_index;
            const tgl = row['date'] * 1;
            const form = moment(tgl).format('DD MMMM YYYY')
            return (
                <tr key={row_index}>
                    {
