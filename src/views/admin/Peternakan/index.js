...
import UpdatePeternakan from './UpdatePeternakan';
...
function Peternakan() {
    ...
    //Data Table
    const objectName = "Peternakan";
    const headings = ['Nama Peternakan', 'Tipe Peternakan', 'Pemilik'];
    const columns = ['name', 'type', 'creator'];
    ...
    const Modal = () => {
        return (
            <div>
                {updateModal && 
                <UpdatePeternakan objectName={objectName} onSuccess={updateSuccess} data={object} modal={updateModal}
    ...
    const fetchData = (keyword = '', limit_ = limit, skip_ = skip, callback) => {
        const q = `query{
                        companiesAll(keyword : "${keyword}", limit : ${limit_}, skip : ${skip_}){
                            totalCount
                            companies {
                                _id
                                name
        ...
        AsyncFetch(q, (res) => {
            setLoading(false);
            setObjects(res.data.data.companiesAll.companies);
            const page = Math.ceil(parseInt(res.data.data.companiesAll.totalCount) / limit_);
            callback(page);
        })
    }
    ...
    const confirmDelete = () => {
        const q = `mutation{
                    deleteCompany(_id : "${deleteObject}"){
                        deleted
                    }
                }
            `;
